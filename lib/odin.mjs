...
// Odin posts "the agent errored" / "the branch push failed" instead of a generic
// "the run failed". PURE: reads the *_OUTCOME envs the action.yml failure step passes.
export function failureSummary(env = {}) {
  // A specific setup error (e.g. "not approved for the shared key") wins over the
  // generic stage message — pre.mjs writes it to $GITHUB_ENV before failing.
  if (env.SETUP_ERROR?.trim()) {
    return env.SETUP_ERROR.trim();
  }
  if (env.PREPARE_OUTCOME === "failure") {
    return "Setup failed before the agent ran — credential lookup, branch checkout, or image download failed. Check the run logs.";
  }
  ...
  if (env.FINALIZE_OUTCOME === "failure") {
    return "The change was made, but the branch push or result reporting failed. Check the run logs.";
  }
